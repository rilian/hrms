class EmployeesMailer < ActionMailer::Base
  ...
  def performance_review(user_id, employees)
    ...
    mail(
      subject: "[HRMS] Performance review plan for next month #{Time.zone.now.strftime('%a, %e %b')} - #{(Time.zone.now + 1.month).strftime('%a, %e %b')}",
      to: @user.email
    )
  end
  ...
  def n_months(user_id, employees, months)
    ...
    @employees = employees
    @months = months
    mail subject: "[HRMS] Employees worked for #{@months} months already", to: @user.email
  end
end
