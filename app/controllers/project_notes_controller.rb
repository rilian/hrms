class ProjectNotesController < ApplicationController
  ...
  def index
    ...
    @project_notes = @project_notes.includes(:project, :updated_by)
  ...
  def create
    @project_note = ProjectNote.new(project_note_params.merge!(updated_by: current_user))
  ...
  def update
    if @project_note.update(project_note_params.merge!(updated_by: current_user))
